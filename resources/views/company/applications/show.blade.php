...
<div class="container-fluid">
  <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
    <h1 class="page-title mb-0">Detail Pelamar</h1>
                    ...
                    <tr>
                      ...
                      <td>
                        {{ $application->user && $application->user->birth_date
                            ? \Carbon\Carbon::parse($application->user->birth_date)->format('d F Y')
                            : '-' }}
                      </td>
                    </tr>
                    <tr>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <span class="{{ $currentStatus['class'] }}">
                      <span class="dot"></span>{{ $currentStatus['label'] }}
                    </span>
                  </td>
                </tr>
                <tr>
            ...
            @if($application->user->portfolio_link || $application->user->facebook || $application->user->instagram || $application->user->linkedin || $application->user->twitter || $application->user->tiktok)
              <div class="d-flex justify-content-center flex-wrap gap-2">
                @if($application->user->portfolio_link)
                  <a href="{{ $application->user->portfolio_link }}" target="_blank" class="btn btn-outline-secondary btn-sm">
                    <i class="fas fa-globe me-1"></i> Portfolio
                  </a>
                @endif
                @if($application->user->facebook)
                  <a href="{{ $application->user->facebook }}" target="_blank" class="btn btn-outline-primary btn-sm"><i class="fab fa-facebook"></i></a>
                @endif
                @if($application->user->instagram)
                  <a href="{{ $application->user->instagram }}" target="_blank" class="btn btn-outline-danger btn-sm"><i class="fab fa-instagram"></i></a>
                @endif
                @if($application->user->linkedin)
                  <a href="{{ $application->user->linkedin }}" target="_blank" class="btn btn-outline-primary btn-sm"><i class="fab fa-linkedin"></i></a>
                @endif
                @if($application->user->twitter)
                  <a href="{{ $application->user->twitter }}" target="_blank" class="btn btn-outline-info btn-sm"><i class="fab fa-twitter"></i></a>
                @endif
                @if($application->user->tiktok)
                  <a href="{{ $application->user->tiktok }}" target="_blank" class="btn btn-outline-dark btn-sm"><i class="fab fa-tiktok"></i></a>
                @endif
              </div>
            @else
...
<script>
function showNoCoverLetterAlert(){ alert('Pelamar tidak mencantumkan surat lamaran.'); }
</script>
@endsection
